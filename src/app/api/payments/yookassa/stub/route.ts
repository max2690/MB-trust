...
export async function GET(request: NextRequest) {
  ...
  // Заглушка страницы оплаты ЮKassa
  const html = `
    <!DOCTYPE html>
    <html lang="ru">
    <head>
      ...
      <style>
        ...
        .amount { font-size: 32px; color: #00D4FF; margin: 20px 0; }
        .button {
          background: #00D4FF;
          color: #0B0B0F;
          border: none;
    ...
    <body>
      ...
      <script>
        function simulatePayment(result) {
          ...
          if (result === 'success') {
            window.location.href = baseUrl + '/api/payments/yookassa/success?paymentId=${paymentId}';
          } else {
            window.location.href = baseUrl + '/api/payments/yookassa/cancel?paymentId=${paymentId}';
          }
        }
      </script>
    </body>
